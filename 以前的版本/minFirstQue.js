...
class minFirstQue{
    constructor(arr, size){
        if(!(arr instanceof Array) || size == undefined || size > arr.length)  throw new Error("请输入正确参数");
        this.heap = arr;
        this.size = size;
        this.buildMinHeap(arr, size);
    }
    ...
    extractMinimum(){//删除根节点，即最小的元素
        var min = this.heap[0], key = this.heap[-- this.size];
    ...
    decreaseKey(i, key){//减小节点的值
        ...
        if(this.heap[i] <= key) throw new Error("输入权值大于等于原本权值");
        this.renewUp(i, key);
    }
    minInsert(key){//插入一个值
    ...
    deleteEle(i){//删除元素
        ...
        var key = this.heap[this.size];//key等于size没减之前的二叉堆的最后一个元素
        ...
        if(key < min){//如果小于子节点，向上更新
            ...
            this.renewDown(i, key);
        }else{//否则把最小的子节点放到i处，然后把key值放到子节点处
            let min = b1 < b2 ? left - 1 : right - 1;//min转换为索引
            this.heap[i] = this.heap[min];
            this.heap[min] = key;
        }
    }
    renewUp(i, key){//最小二叉堆向上更新
        for(let j = i + 1, parent; j > 1;){//要将数组的从0开始的索引转换为二叉树从1开始的索引，所以j等于i加一；在二叉树索引中节点1是根节点，没有父节点了，跳出循环
            parent = floor(j / 2);//定理：在完全二叉树中（二叉堆就是完全二叉树），给定i索引的父节点等于i除以二向下取整
            if(this.heap[parent - 1] > key){//如果父节点值大于key值
    ...
    renewDown(i, key){//最小二叉堆向下更新
        var min;
        for(let j = i + 1, limit = floor(this.size / 2), left, right; j <= limit;){ //要将数组的从0开始的索引转换为二叉树从1开始的索引，所以j等于i加一；limit为二叉堆的最后一个节点的父节点，如果有子节点，则这个节点的索引必然小于等于limit，否则没有子节点，直接退出循环
            left = 2 * j;//给定完全二叉树的节点的i索引，左子节点是2i，右子节点为2i + 1, 转换到数组要减一，因为数组从0开始
            right = left + 1;
            if(right <= this.size) min = this.heap[left - 1] < this.heap[right - 1] ? left : right;//找到子节点中的最小元素
            else min = left;
            if(key <= this.heap[min - 1]){//如果key值小于等于这个子节点最小的值，就不向下更新
                this.heap[j - 1] = key;//直接把key值赋值给j节点
                return;
            }
            this.heap[j - 1] = this.heap[min - 1];//否则j节点赋值为最小子节点的值
            j = min;//然后j等于最小的字节点索引
        }
        this.heap[min - 1] = key; //没有子节点了直接给这个节点赋值
    }
